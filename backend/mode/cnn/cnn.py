...
# 主函数
def cnn_train(paraDict, all_sequences, all_labels, Logging, TimeStart):
    try:
        ...
        # 保存模型
        model_save_path = "backend/Output/" + paraDict['UserName'] + "/" + paraDict['FileOut'] + "/cnn.pth"
...
# 测试模型
def cnn_eval(paraDict, all_sequences, all_labels, Logging, TimeStart):
    try:
        Logging.info("正常：准备运行'cnn'模型测试")
        ...
        # 加载模型
        model.load_state_dict(torch.load("backend/Output/" + paraDict['UserName'] + "/" + paraDict['FileOut'] + "/cnn.pth", weights_only=True))
        model.eval()
